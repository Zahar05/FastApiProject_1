...
def test_send_email_success(mock_delay, client):
    ...
    assert response.json() == {"detail": "Email task started: email-task"}

    mock_delay.assert_called_once_with("user@test.com", "Hello", "World")


def test_send_email_validation_error(client):
    response = client.post("/send_message_to_email", json={})
